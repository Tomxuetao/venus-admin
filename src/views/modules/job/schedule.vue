    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
      ...
      <el-table-column fixed="right" align="center" width="320" label="操作">
        <template v-slot="scope">
          ...
          <el-button v-if="isAuth('sys:schedule:delete')" link size="small" @click="deleteHandle(scope.row.jobId)"
                     style="color: #f56c6c;">删除
          </el-button>
          <el-button v-if="isAuth('sys:schedule:pause')" link size="small" @click="pauseHandle(scope.row.jobId)">暂停
          </el-button>
          <el-button v-if="isAuth('sys:schedule:resume')" link size="small" @click="resumeHandle(scope.row.jobId)">
            恢复
          </el-button>
          <el-button v-if="isAuth('sys:schedule:run')" link size="small" @click="runHandle(scope.row.jobId)">立即执行
          </el-button>
        </template>
      </el-table-column>
    </el-table>
